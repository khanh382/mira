...
- **Telegram:** Sau khi `setWebhook` (va dinh ky bot sync), backend goi `setMyCommands` — user go `/` trong chat se thay cac lenh (menu, list_skills, list_other_skills, new_session, stop, resume, stopall, resumeall). **Khong** co `list_tools` tren menu (trung lap voi `/list_skills`; van go duoc `/list_tools` trong chat).
