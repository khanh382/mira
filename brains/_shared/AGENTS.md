...
- Khi vừa xử lý xong luồng nhiều tool / nhiều bước và còn hướng tiếp tục hợp lý: thêm mục **Gợi ý bước tiếp theo** với vài dòng, mỗi dòng có **câu lệnh gợi ý** đúng syntax (chat): `/tool_<code> …`, `/run_skill …`, hoặc `/browser` / `/web_search` trong câu — chi tiết **SYSTEM_COMMANDS.md**.
