...
export const TASK_RUN_QUEUE = 'task-runs';

/**
 * Hàng đợi chạy task: BullMQ + Redis khi có thể; RAM (in-process) khi Redis không sẵn sàng.
 */
@Injectable()
export class TaskQueueService implements OnModuleInit, OnModuleDestroy {
  ...
  private async bootstrapFromRedis(): Promise<void> {
    ...
    if (!ok) {
      this.logger.warn(
        'Redis không kết nối được — task queue dùng RAM. Sẽ thử lại theo WORKFLOW_REDIS_RETRY_MS.',
      );
    ...
    try {
      ...
      this.logger.log('Task queue: Redis (BullMQ) đã kết nối.');
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      this.logger.warn(`Không khởi tạo BullMQ task queue (${msg}). Dùng RAM.`);
      this.useMemoryFallback = true;
    }
  }
  ...
  private async tryUpgradeToRedis(): Promise<void> {
    ...
    try {
      await this.openRedisQueue();
      this.useMemoryFallback = false;
      this.logger.log('Đã chuyển task queue từ RAM sang Redis (BullMQ).');
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
  ...
  private async runOneJob(runId: string): Promise<void> {
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      this.logger.error(`Task run system error runId=${runId}: ${msg}`);
  ...
  private async openRedisQueue(): Promise<void> {
    ...
    this.queue = new Queue(TASK_RUN_QUEUE, { connection });
    this.worker = new Worker(
      TASK_RUN_QUEUE,
      async (job) => {
        const runId = job.data?.runId as string | undefined;
        if (!runId) {
          this.logger.warn('Task job missing runId');
          return;
        }
        await this.runOneJob(runId);
      },
    ...
    this.worker.on('error', (err) => {
      this.logger.error(`Task BullMQ worker error: ${err.message}`);
    });
  }
